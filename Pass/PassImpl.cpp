...
void ValuePoolCompact::operator()(IRHost &host) {
  auto &pool = host.pool;
  std::vector<unsigned> remap(pool.values.size(),
                              SSAValueHandle::InvalidValueHandle());
  auto value_compare = [](const SSAValue *lhs, const SSAValue *rhs) {
    return lhs->identity.id < rhs->identity.id;
  };
  std::set<SSAValue *, decltype(value_compare)> valid_values(value_compare);
}
    ...
    for (auto &&bb_handle : func->getValidBasicBlock()) {
      ...
      for (auto &&insn_handle : bb->getValidInstruction()) {
        ...
        for (auto arg: insn->args)
          host[arg].user.push_back(host[insn_handle].identity);
      }
    }
  }
}
